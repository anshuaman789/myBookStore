import React from "react";
import banner from "../assets/banner.avif"
function Banner() {
  return (
    <>
      <div className="max-w-screen-2xl container mx-auto md:px-20 px-4 py-10 md:flex">
      <div className="order-2 md:w-1/2">
          <img className=" w-98 h-98 md:ms-3 " src={banner}></img>
        </div>
        <div className=" md:w-1/2 md:order-1  mt-12 md:mt-32 ">
          <div className="space-y-6">
            <h1 className="text-3xl font-bold">
              Hello welcome here to learning something{" "}
              <span className="text-red-500">new everyday!!!</span>{" "}
            </h1>
            <p>
              Lorem, ipsum dolor sit amet consectetur adipisicing elit. Aut
              excepturi repudiandae voluptatem sed. Repudiandae aliquam hic
              adipisci expedita eligendi. In voluptates aliquid maiores dolore
              enim rem voluptate ipsa aliquam sunt.
            </p>
            <label className="px-3 py-2 border rounded-md flex items-center gap-2">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 16 16"
                fill="currentColor"
                className="w-4 h-4 opacity-70"
              >
                <path d="M2.5 3A1.5 1.5 0 0 0 1 4.5v.793c.026.009.051.02.076.032L7.674 8.51c.206.1.446.1.652 0l6.598-3.185A.755.755 0 0 1 15 5.293V4.5A1.5 1.5 0 0 0 13.5 3h-11Z" />
                <path d="M15 6.954 8.978 9.86a2.25 2.25 0 0 1-1.956 0L1 6.954V11.5A1.5 1.5 0 0 0 2.5 13h11a1.5 1.5 0 0 0 1.5-1.5V6.954Z" />
              </svg>
              <input type="text" className="grow outline-none dark:bg-slate-900 dark:text-white" placeholder="Email" />
            </label>
          </div>
          <button className="btn mt-5 btn-secondary">Secondary</button>
        </div>
      </div>
    </>
  );
}

export default Banner;
